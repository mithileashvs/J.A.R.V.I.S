...
def run(
    ...
    model: optional model name override, e.g. from model_selector.py's
    select_model() for a coding-classified request. When omitted,
    _call_model's own default (config.TEXT_MODEL) is used, exactly as
    before this parameter existed — existing callers (workflows/
    exam_prep.py, workflows/hackathon.py, workflows/study_session.py)
    are unaffected since none of them pass this yet.
    """
    prompt = build_prompt(task_system_prompt, history, context_block, max_turns)
    try:
        if model_caller is not None:
            # Injected test/mock callers keep their original 3-arg
            # contract (system_prompt, messages, user_message) — do
            # NOT forward `model` here, since existing tests
            # (test_phase5.py's test_run_uses_injected_model_caller)
            # already inject callers with exactly that signature and
            # no **kwargs; passing model= would break them.
            return model_caller(prompt.system_prompt, prompt.messages, user_message)
        return _call_model(prompt.system_prompt, prompt.messages, user_message, model=model)
